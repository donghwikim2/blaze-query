 ...
 * @since 1.0.0
 */
public class BucketPolicyDataFetcher implements DataFetcher<AwsBucketPolicy>, Serializable {

	public static final BucketPolicyDataFetcher INSTANCE = new BucketPolicyDataFetcher();

	private BucketPolicyDataFetcher() {
	}
							...
							try {
								var bucketPolicy = client.getBucketPolicy(
										GetBucketPolicyRequest.builder().bucket( bucket.name() )
												.build() );
								list.add( new AwsBucketPolicy( account.getAccountId(),
										region.id(),
										bucket.name(),
										bucketPolicy ) );
							}
