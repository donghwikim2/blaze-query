 ...
 * @since 1.0.0
 */
public class RoleDataFetcher implements DataFetcher<AwsRole>, Serializable {
	...
	@Override
	public List<AwsRole> fetch(DataFetchContext context) {
		try {
			List<AwsConnectorConfig.Account> accounts = AwsConnectorConfig.ACCOUNT.getAll( context );
			SdkHttpClient sdkHttpClient = AwsConnectorConfig.HTTP_CLIENT.find( context );
			List<AwsRole> list = new ArrayList<>();
	...
	public DataFormat getDataFormat() {
		return DataFormats.componentMethodConvention( AwsRole.class, AwsConventionContext.INSTANCE );
	}
}
